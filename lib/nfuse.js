...
function buildModuleProject() {
  try {
    for (var _moduleName in rootPackage.dependencies) {
      var _resolve = resolve({ baseDir: cwd, moduleName: _moduleName, preferBrowser: _.flatten(args.browser) });
      ...
      console.log('nfuse: Including module \'' + _moduleName + '\'');
      if (files.length == 0) {
    ...
    _.flatten(args.include).forEach(function (includePath) {
